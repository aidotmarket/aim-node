...
async def session_kill(request):
    ...
    #    Access via ProcessManager._consumer_session_mgr (set by start_consumer, may be None)
    session_mgr = getattr(pm, '_consumer_session_mgr', None)
    ...
    #    Access via ProcessManager._provider_handler (set by start_provider, may be None)
    provider_handler = getattr(pm, '_provider_handler', None)
...
### C.3 ProcessManager Wiring Changes (Required)

The current `ProcessManager` implementation creates session-layer objects as local variables or private fields that are inaccessible to management handlers. Two changes are required:
...
`ProcessManager.start_consumer()` already stores SessionManager on `self._consumer_session_mgr` (process.py ~line 126). No change needed — just use the existing private attribute.
...
def __init__(self, ...):
    # ... existing ...
    self._provider_handler: ProviderSessionHandler | None = None  # NEW
    # _consumer_session_mgr already initialized
```
...
- Tests: 14+ (restart cycle, reload config, reload with bad config → facade=None, lock then unlock, rotate + verify fingerprint, kill valid session consumer-side, kill valid session provider-side, kill invalid session → 404, ProcessManager stores/clears provider_handler)
...
5. **Forward:** `POST /allie/chat/agentic` via `MarketplaceFacade.post()` with `json_body={message, context, node_id, conversation_id, allowed_tools}`
...
| C: Provider Lifecycle + Security Ops + Session Kill | 5 | Extensions to `routes.py`, changes to `process.py` | 14 |
| D: Static Files + SPA + allAI | 2 (+ SPA fallback) | `management/allai.py`, updates to `app.py` | 12 |
| **Total** | **16 + 1 WS + SPA** | | **59** |
...
10. 59+ tests passing, CI green
11. All error responses use normalized format (ErrorCode + make_error)
