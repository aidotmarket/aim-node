...
#
# Adapted from the vectorAIz release.sh pattern.
# =============================================================================
set -euo pipefail
...
IMAGE="ghcr.io/aidotmarket/aim-node"
COMPOSE_FILE="docker-compose.aim-node.yml"
COMPOSE_VAR="AIM_NODE_VERSION"
...
preflight() {
  ...
  # GHCR access (best-effort login if manifest not accessible)
  if ! "$DOCKER" manifest inspect "$IMAGE:latest" &>/dev/null; then
    ...
    if [[ -n "$token" ]]; then
      echo "$token" | "$DOCKER" login ghcr.io -u aidotmarket --password-stdin \
        && pass "Logged into GHCR" \
        || info "GHCR login failed — continuing (first release may have no :latest yet)"
    else
      info "No GITHUB_TOKEN — skipping GHCR login (OK for first release)"
    fi
  else
...
update_compose() {
  local ver="$1"
  sed -i '' "s/${COMPOSE_VAR}:-[^}]*/${COMPOSE_VAR}:-v${ver}/" "$COMPOSE_FILE"
  grep -q "${COMPOSE_VAR}:-v${ver}" "$COMPOSE_FILE" || die "sed failed to update $COMPOSE_FILE"
...
wait_for_image() {
  ...
  die "Timed out (10 min) waiting for $IMAGE:${tag}" \
      "Check GitHub Actions: gh run list --workflow=docker-build.yml"
}
...
cmd_promote() {
  ...
  pass "Multi-arch retag complete (v${ver} + latest)"
